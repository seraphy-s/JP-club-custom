...
class BasicList extends Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'listBasicList/fetchAllUser',
    });
  }
  ...
  setRejectReason = () => {
    ...
    form.validateFields((err, fieldsValue) => {
      if (err) {
        console.log(err, 'validate error', fieldsValue);
      }
      dispatch({
        type: 'listBasicList/setRejectReason',
        payload: {
          _id: current._id,
          reject_reason: fieldsValue.reject_reason,
        },
      }).then(function() {
        _this.setState({
          rejectVisible: false,
    ...
    const columns = [
      {
        ...
        dataIndex: 'uid',
        width: 40,
        filters: this.props.listBasicList.users.map(user => {
          return {
            text: user.name,
            value: user._id,
          };
        }),
        onFilter: (value, record) => record.uid.indexOf(value) === 0,
        render: (uid, record) => (
          <div style={{ wordWrap: 'break-word', wordBreak: 'break-all' }}>{uid}</div>
        ),
      },
      {
        title: '推广文案',
            ...
            case '3': // 3 审核中
              return (
                <>
                  ...
                  <a
                    onClick={() => {
                      this.setState({ rejectVisible: true, current: item });
                    }}
                  >
                    驳回
                  </a>
    ...
    const expandedRowRender = record => {
      const expandedColumns = [
        ...
        // },
        {
          title: '到期时间',
          dataIndex: 'end_time',
          render: (end_time, item) => {
            // console.log(end_time, 'endtime', item)
            if (!end_time) {
              return <></>;
            }
            return <span>{moment(end_time).format('YYYY-MM-DD')}</span>;
          },
        },
      ];
    ...
    return (
      <>
            ...
            >
              <FormItem {...formItemLayout} label={'服务费比例'}>
                {getFieldDecorator('fee_propo', {
                  initialValue: current.fee_propo,
                  rules: [
                    {
                      required: true,
                      message: '请输入服务费比例',
        ...
        <Modal
          ...
          onOk={() => this.setRejectReason()}
          confirmLoading={this.props.listBasicList.rejectLoading}
        ...
        >
          ...
          <FormItem {...formItemLayout} label={'驳回原因'}>
            {getFieldDecorator('reject_reason', {
              initialValue: current.reject_reason || '',
              rules: [
                {
                  required: true,
                  message: '请输入驳回原因',
